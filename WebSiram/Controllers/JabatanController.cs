...
namespace WebSiram.Controllers
{
    public class JabatanController : Controller
    {
        ...
        public JabatanController(ILogger<JabatanController> logger, IConfiguration config)
        {
            _logger = logger;
            _config = config;
        }
        ...
        public async Task<ActionResult> Index()
        {
            ...
            using (var client = new HttpClient(clientHandler))
            {
                ...
                HttpResponseMessage Res = await client.GetAsync("api/Jabatan");
                if (Res.IsSuccessStatusCode)
                {
                    var Response = Res.Content.ReadAsStringAsync().Result;
                    jabatan = JsonConvert.DeserializeObject<List<Jabatan>>(Response);
                }
                else
                {
